(()=>{
  ...
  const en={
    ...
    aboutThinkLabel:'HOW WE THINK',aboutBuildLabel:'WHAT WE BUILD',aboutWhoLabel:'WHO IT IS FOR',drawerTagline:'Technology that moves business forward.',virtualAssistant:'Virtual Assistant',supportAvailable:'Support available',chatWelcome:'Hi. I am the DONBEX virtual assistant. I can guide you through solutions, products or how to start your project.',chipStart:'How do I start my project?',chipSolutions:'What can DONBEX do?',chipProducts:'Our products',chipRadio:'Radio App',chipSupport:'Talk to support'
  };
  ...
  const answers={
   es:{
    ...
    support:'Perfecto. Te voy a mostrar el formulario de soporte dentro del chat para que no tengas que salir de DONBEX. Tu consulta se enviará directamente al equipo de soporte.'
   },
   en:{
    ...
    support:'Perfect. I will show the support form inside the chat so you do not need to leave DONBEX. Your message will be sent directly to the support team.'
   }
  };
  if(lang==='en'){
    const chatEnglish={automation:'Automate my business',ai:'AI Assistant / Chatbot',web:'Professional website',pricing:'Pricing & quote'};
    Object.entries(chatEnglish).forEach(([k,v])=>{const b=$(`[data-chat-topic="${k}"]`);if(b){const n=b.querySelector('span')?.outerHTML||'';b.innerHTML=n+v}});
    if($('#chatContext')) $('#chatContext').textContent='Intelligent guidance for your business';
    if(chatInput) chatInput.placeholder='Ask about your project, product or solution...';
    const foot=$('.chat-foot small'); if(foot) foot.textContent='Initial guidance · Human support available';
  }else if(chatInput){chatInput.placeholder='Pregunta por tu proyecto, producto o solución...'}
  ...
  function showSupportForm(){
    ...
    supportView.innerHTML=lang==='en'?`<form id="chatSupportForm" class="chat-support-form"><div class="support-form-head"><span>DONBEX SUPPORT</span><b>Talk to our team</b><p>Tell us what you need. Your request goes directly to DONBEX support.</p></div><div class="support-grid"><label>Full name<input name="name" required autocomplete="name" placeholder="Your name"></label><label>Email<input name="email" type="email" required autocomplete="email" placeholder="you@email.com"></label><label>Phone / WhatsApp <small>optional</small><input name="phone" autocomplete="tel" placeholder="+1 ..."></label><label>Business <small>optional</small><input name="business" autocomplete="organization" placeholder="Business name"></label><label class="full">What do you need?<select name="category"><option>General support</option><option>Start a project</option><option>Radio App</option><option>Automation</option><option>AI assistant / Chatbot</option><option>Website / Platform</option><option>Other</option></select></label><label class="full">Message<textarea name="message" required placeholder="Tell us how we can help you..."></textarea></label></div><div class="support-actions"><button type="submit" class="support-send">Send request <span>→</span></button></div><small class="support-status" id="chatSupportStatus"></small></form>`:`<form id="chatSupportForm" class="chat-support-form"><div class="support-form-head"><span>SOPORTE DONBEX</span><b>Habla con nuestro equipo</b><p>Cuéntanos qué necesitas. Tu solicitud llega directamente al soporte de DONBEX.</p></div><div class="support-grid"><label>Nombre y apellido<input name="name" required autocomplete="name" placeholder="Tu nombre"></label><label>Correo electrónico<input name="email" type="email" required autocomplete="email" placeholder="tu@correo.com"></label><label>Teléfono / WhatsApp <small>opcional</small><input name="phone" autocomplete="tel" placeholder="+593 ..."></label><label>Negocio / empresa <small>opcional</small><input name="business" autocomplete="organization" placeholder="Nombre del negocio"></label><label class="full">¿Qué necesitas?<select name="category"><option>Soporte general</option><option>Crear un proyecto</option><option>Radio App</option><option>Automatización</option><option>Asistente IA / Chatbot</option><option>Página web / Plataforma</option><option>Otro</option></select></label><label class="full">Tu consulta<textarea name="message" required placeholder="Cuéntanos cómo podemos ayudarte..."></textarea></label></div><div class="support-actions"><button type="submit" class="support-send">Enviar solicitud <span>→</span></button></div><small class="support-status" id="chatSupportStatus"></small></form>`;
    const wrap=$('#chatSupportForm',supportView);
    wrap?.addEventListener('submit',async e=>{e.preventDefault();if(!wrap.reportValidity())return;const b=wrap.querySelector('.support-send'),st=wrap.querySelector('.support-status');b.disabled=true;st.textContent=lang==='en'?'Sending securely...':'Enviando de forma segura...';try{const fd=new FormData(wrap);const r=await fetch('/api/donbex-support',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'support',name:fd.get('name'),email:fd.get('email'),phone:fd.get('phone'),business:fd.get('business'),category:fd.get('category'),message:fd.get('message'),language:lang})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'send');hideSupportView();typingThen(lang==='en'?'Your request was sent successfully to DONBEX support. A specialist will contact you by email.':'Tu solicitud fue enviada correctamente al soporte de DONBEX. Un especialista se comunicará contigo por correo.');}catch(err){st.textContent=lang==='en'?'We could not send your request. Check your information and try again.':'No pudimos enviar tu solicitud. Revisa tus datos e inténtalo nuevamente.';b.disabled=false;}})
  }
  function topic(t){const text=answers[lang][t]||answers[lang].start;typingThen(text,()=>{if(t==='support')showSupportForm()})}
  $('#chatChips')?.addEventListener('click',e=>{const b=e.target.closest('[data-chat-topic]');if(!b)return;addMsg(b.textContent.trim(),'user');topic(b.dataset.chatTopic)});
