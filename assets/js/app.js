(()=>{
  ...
  function showSupportForm(){
    ...
    supportView.innerHTML=lang==='en'?`<div class="support-title">Talk to support</div><div class="support-note">If your request needs human review, you can send it directly to DONBEX support from here. You do not need to open another app.</div><form id="chatSupportForm" class="chat-support-form"><p>Complete your information and message. It will be sent directly to DONBEX support.</p><div class="support-grid"><input name="name" required autocomplete="name" placeholder="Full name"><input name="email" type="email" required autocomplete="email" placeholder="Email"><input class="full" name="phone" autocomplete="tel" placeholder="Phone / WhatsApp (optional)"><input class="full" name="business" autocomplete="organization" placeholder="Business / company (optional)"><select class="full" name="category"><option>General support</option><option>Start a project</option><option>Radio App</option><option>Automation</option><option>AI assistant / Chatbot</option><option>Website / Platform</option><option>Other</option></select><textarea class="full" name="message" required placeholder="Write your request"></textarea></div><button type="submit" class="support-send">Send request</button><small class="support-status" id="chatSupportStatus"></small></form>`:`<div class="support-title">Hablar con soporte</div><div class="support-note">Si tu consulta necesita revisión humana, puedes enviarla directamente al soporte de DONBEX desde aquí. No necesitas abrir otra aplicación.</div><form id="chatSupportForm" class="chat-support-form"><p>Completa tus datos y tu consulta. El mensaje se enviará directamente a soporte DONBEX.</p><div class="support-grid"><input name="name" required autocomplete="name" placeholder="Nombre y apellido"><input name="email" type="email" required autocomplete="email" placeholder="Correo"><input class="full" name="phone" autocomplete="tel" placeholder="Teléfono / WhatsApp (opcional)"><input class="full" name="business" autocomplete="organization" placeholder="Negocio / empresa (opcional)"><select class="full" name="category"><option>Soporte general</option><option>Crear un proyecto</option><option>Radio App</option><option>Automatización</option><option>Asistente IA / Chatbot</option><option>Página web / Plataforma</option><option>Otro</option></select><textarea class="full" name="message" required placeholder="Escribe tu consulta"></textarea></div><button type="submit" class="support-send">Enviar consulta</button><small class="support-status" id="chatSupportStatus"></small></form>`;
    const wrap=$('#chatSupportForm',supportView);
    wrap?.addEventListener('submit',async e=>{e.preventDefault();if(!wrap.reportValidity())return;const b=wrap.querySelector('.support-send'),st=wrap.querySelector('.support-status');b.disabled=true;st.textContent=lang==='en'?'Sending...':'Enviando...';try{const fd=new FormData(wrap);const r=await fetch('/api/donbex-support',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'support',name:fd.get('name'),email:fd.get('email'),phone:fd.get('phone'),business:fd.get('business'),category:fd.get('category'),message:fd.get('message'),language:lang})});const data=await r.json().catch(()=>({}));if(!r.ok)throw new Error(data.error||'send');hideSupportView();typingThen(lang==='en'?'Your request was sent successfully to DONBEX support. A specialist will contact you by email.':'Tu consulta fue enviada correctamente a soporte DONBEX. Un especialista se comunicará contigo por correo.');}catch(err){st.textContent=lang==='en'?'We could not send your request. Please try again.':'No pudimos enviar tu consulta. Inténtalo nuevamente.';b.disabled=false;}})
  }
